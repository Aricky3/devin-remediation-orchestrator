...
def test_completed_when_finished_with_pr():
    state = derive_state(
...
def test_completed_via_structured_output_even_without_pr_list():
    state = derive_state(
        status="finished", status_detail=None, pull_requests=[],
...
def test_failed_when_terminal_without_pr():
    state = derive_state(status="expired", status_detail=None, pull_requests=[], structured_output=None)
